<x-app-layout>
            ...
            <div>
                ...
                <nav class="flex mb-2" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-2">
                        <li>
                            <a href="{{ route('admin.modul.index') }}" class="text-sm font-medium text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-white">Module Management</a>
                        </li>
                        <li>
    ...
    {{-- Toast Notification (keep as is, this is good) --}}
    @if (session('success'))
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-2xl border border-gray-200 dark:border-gray-700/50">
                <div class="p-6 md:p-8 space-y-6">
                    <!-- Material Data Table (New Design) -->
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            <thead class="border-b border-gray-200 dark:border-gray-700">
                                <tr>
                                    <th scope="col" class="w-12 px-6 py-3"></th>
                                    <th scope="col" class="w-16 px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Order</th>
                                    <th scope="col" class="px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Material Title</th>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div x-data="{ open: false, deleting: false }" class="relative">
                                                ...
                                                <div x-show="open" @click.away="open = false" x-transition x-cloak class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl py-1 ring-1 ring-black ring-opacity-5 z-20 text-left">
                                                    <a href="{{ route('admin.modul.materi.edit', [$modul, $item]) }}" class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">Edit Material</a>
                                                    <div class="border-t border-gray-100 dark:border-gray-700 my-1"></div>
                                                    <button @click="deleting = true; open = false" class="flex items-center gap-3 w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20">Delete</button>
                                                </div>
                                                ...
                                                <div x-show="deleting" x-cloak x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                                                    <div @click.away="deleting = false" class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 md:p-8 max-w-md w-full">
                                                        <div class="text-center">
                                                            ...
                                                            <h3 class="mt-4 text-xl font-bold text-gray-900 dark:text-white">Confirm Deletion</h3>
                                                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Are you sure you want to delete the material <span class="font-semibold">"{{ $item->title }}"</span>? This action cannot be undone.</p>
                                                        </div>
                                                        <div class="mt-6 grid grid-cols-2 gap-4">
                                                            <button @click="deleting = false" type="button" class="w-full px-4 py-2.5 text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">Cancel</button>
                                                            <form action="{{ route('admin.modul.materi.destroy', [$modul, $item]) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="w-full px-4 py-2.5 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors">Yes, Delete</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-24 text-center">
                                            <div class="flex flex-col items-center max-w-sm mx-auto">
                                                ...
                                                <h3 class="mt-4 text-lg font-semibold text-gray-800 dark:text-gray-200">No Materials Yet</h3>
                                                <p class="mt-1 text-sm text-gray-500">Start by adding a new material for this module. The materials you add will appear here.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
