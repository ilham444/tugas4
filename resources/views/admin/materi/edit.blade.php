                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700" x-data="{ showUploader: false }">
                            ...
                            <div x-show="!showUploader" class="space-y-4">
                                <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                    ...
                                    <div>
                                        <a href="{{ Storage::url($materi->file_path) }}" target="_blank" class="text-sm font-semibold text-gray-800 dark:text-gray-200 hover:underline">{{ Str::limit(basename($materi->file_path), 25) }}</a>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">File saat ini. Klik untuk melihat.</p>
                                    </div>
                                </div>
                            ...
                            <div x-show="showUploader" x-cloak>
                                <div x-data="fileDrop()" class="relative flex flex-col items-center justify-center w-full p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-center cursor-pointer hover:border-indigo-500 dark:hover:border-indigo-400 transition-colors">
                                    ...
                                    <div x-show="!fileName" class="z-10"><p class="text-sm text-gray-600 dark:text-gray-300">Seret file baru ke sini</p></div>
                                    <div x-show="fileName" class="z-10 text-sm font-medium text-gray-800 dark:text-gray-200" x-text="fileName"></div>
                                </div>
                                <p class="text-xs text-gray-500 mt-2">Kosongkan jika tidak ingin mengubah file saat ini.</p>
                                <button @click="showUploader = false" type="button" class="mt-2 text-sm font-semibold text-gray-600 dark:text-gray-400 hover:underline">Batal ganti file</button>
