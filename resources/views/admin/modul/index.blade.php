<x-app-layout>
    <x-slot name="header">
        {{-- Header is already great, no changes here --}}
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
                <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ __('Module Management') }}
                </h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage, filter, and search all modules on the platform.</p>
            </div>
    ...
    {{-- Toast Notification (keep as is, this is already good) --}}
    @if (session('success'))
                        ...
                        <form action="{{ route('admin.modul.index') }}" method="GET">
                            <div class="flex flex-col md:flex-row items-end justify-between gap-4">
                                <div class="w-full md:w-1/2 lg:w-2/5">
                                    <label for="search" class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 block">Search Module</label>
                                    <div class="relative flex items-center">
                                        ...
                                        <input type="text" name="search" id="search" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 focus:ring-indigo-500 focus:border-indigo-500 transition-all" placeholder="Search by title..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="w-full md:w-auto flex items-end gap-3">
                                    <div>
                                        <label for="kategori" class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 block">Category</label>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            <thead class="bg-gray-50 dark:bg-gray-700/50">
                                <tr>
                                    <th scope="col" class="w-5/12 px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Title</th>
                                    <th scope="col" class="w-2/12 px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Category</th>
                                    <th scope="col" class="w-2/12 px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Details</th>
                                    <th scope="col" class="w-2/12 px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Created</th>
                                    <th scope="col" class="w-1/12 relative px-6 py-3"><span class="sr-only">Actions</span></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="group transition-all duration-200 ease-in-out hover:shadow-md hover:bg-gray-50 dark:hover:bg-gray-900/50 hover:-translate-y-px">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300 align-top">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span>{{ $item->estimated }} minutes</span>
                                        </div>
                                        <div class="flex items-center gap-2 mt-1">
                                            ...
                                            <span>{{ $item->materis->count() }} materials</span>
                                        </div>
                                    </td>
                                            ...
                                            <div x-show="deleting" x-cloak @keydown.escape.window="deleting = false" class="fixed inset-0 bg-black bg-opacity-50 z-30 flex items-center justify-center">
                                                <div @click.away="deleting = false" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100" class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md">
                                                    ...
                                                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                                        Are you sure you want to delete the module <strong class="font-semibold text-gray-700 dark:text-gray-300">"{{ $item->title }}"</strong>? All materials within it will also be deleted. This action cannot be undone.
                                                    </p>
                                                    <form action="{{ route('admin.modul.destroy', $item) }}" method="POST" class="mt-6 flex justify-end gap-3">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" @click="deleting = false" class="py-2 px-4 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 font-semibold rounded-lg transition-colors">Cancel</button>
                                                        <button type="submit" class="py-2 px-4 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-colors">Yes, Delete</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <tr>
                                    <td colspan="5" class="text-center py-16 px-6">
                                        ...
                                        <h3 class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">No modules found</h3>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Try changing your filters or search keywords.</p>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
